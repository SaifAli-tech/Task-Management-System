...
const DepartmentsPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await apiClient.get(
        `/departments/pagedata?page=${page}&take=${rowsPerPage}&order=${sortDirection}&search=${searchTerm}`
      );
      setDepartments(response.data.departments);
      setCount(response.data.meta.pageCount);

      router.replace(
        `/departments?page=${page}&take=${rowsPerPage}&order=${sortDirection}&search=${searchTerm}`
      );
    } catch (error) {
    ...
    if (deleteId && deleteName) {
      try {
        ...
        Swal.fire({
          ...
          color: "green",
          title: "Success",
          text: `Department "${deleteName}" deleted successfully.`,
        });
        fetchData();
        closeConfirmDialog();
      } catch (error) {
  ...
  const openConfirmDialog = (departmentId, departmentName) => {
  ...
  return (
    <div className="w-full bg-white rounded-lg shadow-lg p-4">
      ...
      <Dialog open={confirmDialogOpen} onClose={closeConfirmDialog}>
        <DialogTitle className="text-center bg-purple-800 text-white font-bold  m-1 rounded-md">{`Delete Department "${deleteName}"`}</DialogTitle>
              ...
              {departments.map((department, index) => (
                <TableRow key={department._id}>
                  ...
                  <TableCell className="font-bold text-base text-center">
                    ...
                    <IconButton
                      onClick={() =>
                        openConfirmDialog(department._id, department.name)
                      }
                    >
